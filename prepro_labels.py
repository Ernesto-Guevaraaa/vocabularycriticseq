...
def get_image_id(filename):
  img_ind = int(filename.strip().split('_')[-1].split('.')[0][-6:])
  image_id="%02d/%d" % (img_ind / 10000, img_ind)
  return image_id
...
def generate_annfile(imgs,params) :
    dataset = imgs
    ref_dataset = {}
    annotations = []
    images = []
    ...
    for item in dataset :
        img_id = item['cocoid']
        image = {}
        image['file_name'] = item['filename']
        image['id'] = img_id
        images.append(image)
        for sentence in item['sentences'] :
            cap=' '.join(sentence['tokens'])
            if len(cap) == 0 :
                continue
            ann = {}
            ann['caption'] = cap
    ...
    json.dump(ref_dataset, open(params['annfile_json'], 'w'))
...
def main(params):
  ...
  for i,img in enumerate(imgs):
    ...
    jimg['image_id'] = get_image_id(img['filename'])
...
if __name__ == "__main__":
  ...
  parser.add_argument('--annfile_json', default='ann.h5', help='annotation json file')
