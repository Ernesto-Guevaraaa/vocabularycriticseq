...
from misc.rewards import init_scst_scorer, get_scst_reward
...
def train(opt):
    ...
    scst_crit = utils.RewardCriterion()
    scorer=init_scst_scorer(opt)
    ...
    for epoch in range(start_epoch, opt.max_epochs + 1):
        ...
        if True or epoch % opt.save_every == 0 :
            ...
            lang_val_loss, act_val_loss, predictions, lang_stats = eval_utils.eval_split(cnn_model, model, lang_crit, opt, eval_kwargs)
        ...
        for batch in range(batches_per_epoch):
            ...
            if opt.use_img != 0 :
                ...
                try:
                    gen_result, sample_logprobs = model_.sample(fc_feats, att_feats, {'sample_max': opt.rl_greedy})
                except:
                    gen_result, sample_logprobs = model_.module.sample(fc_feats, att_feats, {'sample_max':opt.rl_greedy})

                reward_list,base_score,explore_score= get_scst_reward(model_, fc_feats, att_feats, sample_logprobs,data, gen_result,scorer, opt)
                loss_scst=scst_crit(sample_logprobs, gen_result, Variable(torch.from_numpy(reward_list).float().cuda(), requires_grad=False))
            ...
            if iteration % 25 == 0 :
                print("iter {} (epoch {}), avg_reward = {:.3f}, base_score={:.3f}, explore_score={:.3f}, time/batch = {:.3f}, time2/batch = {:.3f}" \
