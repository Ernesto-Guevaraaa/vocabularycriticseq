...
from misc.rewards import get_wc_reward
from scorer import scorer_setup
    ...
    while tokill() == False:
        for batch, data in enumerate(dataset_generator):
            ...
            for i in range(opt.batch_size):
                data['labels'][i * opt.seq_per_img: (i + 1) * opt.seq_per_img, 1: opt.seq_length + 1] = \
                data['seq_list'][i]
...
def train(opt):
    ...
    wc_crit = utils.RewardCriterion_WC()
    scst_crit = utils.RewardCriterion_SCST_with_WC()

    scorer=scorer_setup(opt)
    ...
    if opt.use_img != 0 and opt.finetune_cnn_after != -1 :
        ...
        if opt.gpu_num > 1:
            cnnrnn_model = torch.nn.DataParallel(cnnrnn_model)
    else :
        if opt.gpu_num > 1:
    ...
    for epoch in range(start_epoch, opt.max_epochs + 1):
        ...
        #if False:
        if True or epoch % opt.save_every == 0 :
            eval_kwargs = {'split': 'val',
                            'dataset': opt.input_json}
            eval_kwargs.update(vars(opt))
            lang_val_loss, act_val_loss, predictions, lang_stats = eval_utils.eval_split(cnn_model, model_, lang_crit, opt, eval_kwargs)
            ...
            if opt.language_eval == 1:
                ...
                if opt.tar_metric=='CIDEr':
                    ...
                    key_score='METEOR'
                print('key_score: %s'%(key_score))
                current_score = lang_stats[key_score]
            else:
            ...
            if opt.save_every and epoch % opt.save_every == 0:
            ...
            if epoch > opt.learning_rate_decay_start and opt.learning_rate_decay_start >= 0:
                ...
                opt.current_lr = opt.learning_rate
            # Assign alpha   
            if epoch > opt.alpha_decay_start and opt.alpha_decay_start >= 0:
                frac = (epoch - opt.alpha_decay_start) // opt.alpha_decay_every
                decay_factor = opt.alpha_decay_rate * frac
                alpha_=opt.alpha + decay_factor
                opt.current_alpha=min(opt.alpha_max,alpha_)
            else:
                opt.current_alpha = opt.alpha 
            print('alpha:%f'%(opt.current_alpha))
        ...
        for batch in range(batches_per_epoch):
            ...
            if opt.use_img != 0 :
                seq_output = cnnrnn_model(images, labels)
                lang_loss = lang_crit(seq_output, labels[:, 1:], masks[:, 1:])
                loss = lang_loss
            else :
                gen_result, all_logprobs, sample_logprobs, = model_.sample_WC(fc_feats, att_feats, {'sample_max': opt.rl_greedy})
               
                # reward, base_cider, explore_cider = get_self_critical_reward(model_, fc_feats, att_feats, data, gen_result)
                reward_list,reward_scst_list,base_score,explore_score= get_wc_reward(model_, fc_feats, att_feats, all_logprobs,data, gen_result,scorer,opt)
                loss_wc = wc_crit(all_logprobs, gen_result,sample_logprobs, Variable(torch.from_numpy(reward_list).float().cuda(), requires_grad=False))
                if opt.use_scst!=0:
                    loss_scst=scst_crit(all_logprobs, gen_result, Variable(torch.from_numpy(reward_scst_list).float().cuda(), requires_grad=False))
                else:
                    loss_scst=0
                
                lang_loss=opt.current_alpha*loss_wc+loss_scst

            loss_ = lang_loss / opt.iter_times
            loss_.backward()
            ...
            if iteration % 25 == 0 :
                ...
                # print("iter {} (epoch {}), avg_reward = {:.3f}, base_cider = {:.3f}, explore_cider = {:.3f},  time/batch = {:.3f}, time2/batch = {:.3f}" \
                #     .format(iteration, epoch, np.mean(reward[:,0]), base_cider, explore_cider, end - start, read_data_time))
                print("iter {} (epoch {}), avg_reward = {:.7f}, base_score={:.3f}, explore_score={:.3f}, time/batch = {:.3f}, time2/batch = {:.3f}" \
                    .format(iteration, epoch, np.mean(reward_list)*1000,np.mean(base_score),np.mean(explore_score), end - start, read_data_time))
            ...
            if (iteration % opt.losses_log_every == 0):
                if tf is not None:
                    ...
                    add_summary_value(tf_summary_writer, 'avg_reward', np.mean(reward_list[:,0]), iteration)
                    tf_summary_writer.flush()
